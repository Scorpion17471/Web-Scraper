...
class AmzSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for product in response.css("div.s-asin"):
            ...
            if item["regular_price"] != None:
                item["regular_price"] = item["regular_price"][1:]
            ...
            item["review_count"] = "".join(product.css("div.puis-list-col-right > div > div > div[data-cy=reviews-block] > div > span:last-child > div > a > span::text").get().split(','))
            yield item
